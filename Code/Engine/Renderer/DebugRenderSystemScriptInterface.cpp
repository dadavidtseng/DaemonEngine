...
//----------------------------------------------------------------------------------------------------
DebugRenderSystemScriptInterface::DebugRenderSystemScriptInterface(DebugRenderAPI* debugRenderAPI, CameraStateBuffer* cameraStateBuffer)
    : m_debugRenderAPI(debugRenderAPI)
    , m_cameraStateBuffer(cameraStateBuffer)
{
...
ScriptMethodResult DebugRenderSystemScriptInterface::ExecuteRenderWorld(ScriptArgs const& args)
{
    ...
    try
    {
        // Extract cameraId and resolve to Camera* via CameraStateBuffer
        int cameraId = ScriptTypeExtractor::ExtractInt(args[0]);

        if (!m_cameraStateBuffer)
        {
            return ScriptMethodResult::Error("CameraStateBuffer not available for renderWorld");
        }

        Camera const* camera = m_cameraStateBuffer->GetCameraById(static_cast<unsigned long long>(cameraId));
...
ScriptMethodResult DebugRenderSystemScriptInterface::ExecuteRenderScreen(ScriptArgs const& args)
{
    ...
    try
    {
        ...
        if (!m_cameraStateBuffer)
        {
            return ScriptMethodResult::Error("CameraStateBuffer not available for renderScreen");
        }
