...
// 全域變數
//----------------------------------------------------------------------------------------------------
LogSubsystem* g_theLogSubsystem = nullptr;
    ...
    , lineNumber(line)
{
    if (g_theLogSubsystem) {
        timestamp = g_theLogSubsystem->GetCurrentTimestamp();
        threadId = g_theLogSubsystem->GetCurrentThreadId();
    }
}
...
void FileOutputDevice::WriteLog(const LogEntry& entry)
{
    ...
    if (m_logFile.is_open()) {
        ...
        switch (entry.verbosity) {
            ...
            case eLogVerbosity::Display:    verbosityStr = "Display"; break;
            case eLogVerbosity::Log:        verbosityStr = "Log"; break;
            case eLogVerbosity::Verbose:    verbosityStr = "Verbose"; break;
            case eLogVerbosity::VeryVerbose: verbosityStr = "VeryVerbose"; break;
        }
...
void OnScreenOutputDevice::WriteLog(const LogEntry& entry)
{
    Rgba8 color;
    switch (entry.verbosity) {
        case eLogVerbosity::Fatal:      color = Rgba8::RED; break;
        case eLogVerbosity::Error:      color = Rgba8(255, 100, 100, 255); break;
        case eLogVerbosity::Warning:    color = Rgba8::YELLOW; break;
        case eLogVerbosity::Display:    color = Rgba8::GREEN; break;
        default:                        color = Rgba8::WHITE; break;
    }
...
void DevConsoleOutputDevice::WriteLog(const LogEntry& entry)
{
    if (g_theDevConsole) {
        ...
        g_theDevConsole->AddLine(lineType, "[" + entry.category + "] " + entry.message);
    }
}

bool DevConsoleOutputDevice::IsAvailable() const
{
    return g_theDevConsole != nullptr;
}

//----------------------------------------------------------------------------------------------------
// LogSubsystem 實作
//----------------------------------------------------------------------------------------------------
LogSubsystem::LogSubsystem(const sLogSubsystemConfig& config)
    : m_config(config)
{
    s_instance = this;
}
...
void LogSubsystem::Startup()
{
    ...
    // 記錄啟動訊息
    LogMessage("LogCore", eLogVerbosity::Display, "DaemonEngine LogSubsystem 已啟動");
...
{
    // 記錄關閉訊息
    LogMessage("LogCore", eLogVerbosity::Display, "DaemonEngine LogSubsystem 正在關閉...");
