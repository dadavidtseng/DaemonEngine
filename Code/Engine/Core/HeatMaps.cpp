...
void TileHeatMap::SetAllValues(float const value) const
{
    int const tileNums = GetTileNums();
...
void TileHeatMap::SetValueAtCoordsIfSmallerThanCurrent(int const tileX, int const tileY, float value) const
{
    if (IsOutOfBounds(tileX, tileY))
        ERROR_AND_DIE("tileCoords is out of bounds")

    // Correct index calculation for row-major order
    int const tileIndex = GetTileIndex(tileX, tileY);

    // Check and update the value if the new value is smaller
    if (value < m_values[tileIndex])
    {
...
{
    IntVec2 const tileCoord(tileX, tileY);

    if (IsOutOfBounds(tileCoord))
        ERROR_AND_DIE("tileCoords is out of bounds")

    int const tileIndex = GetTileIndex(tileCoord);
