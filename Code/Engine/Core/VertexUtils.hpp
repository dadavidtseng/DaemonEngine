...
void AddVertsForDisc2D(VertexList_PCU& verts, Vec2 const& discCenter, float discRadius, Rgba8 const& color);
void AddVertsForDisc3D(VertexList_PCU& verts, Vec3 const& discCenter, float discRadius, Vec3 const& normalDirection, Rgba8 const& color);
void AddVertsForDisc2D(VertexList_PCU& verts, Disc2 const& disc, Rgba8 const& color);
void AddVertsForLineSegment2D(VertexList_PCU& verts, Vec2 const& startPosition, Vec2 const& endPosition, float thickness, bool isInfinite, Rgba8 const& color);
void AddVertsForLineSegment2D(VertexList_PCU& verts, LineSegment2 const& lineSegment, float thickness, bool isInfinite, Rgba8 const& color);
void AddVertsForTriangle2D(VertexList_PCU& verts, Vec2 const& ccw0, Vec2 const& ccw1, Vec2 const& ccw2, Rgba8 const& color);
void AddVertsForTriangle2D(VertexList_PCU& verts, Triangle2 const& triangle, Rgba8 const& color);
void AddVertsForAABB2D(VertexList_PCU& verts, AABB2 const& aabb2Box, Rgba8 const& color, Vec2 const& uvMins = Vec2::ZERO, Vec2 const& uvMaxs = Vec2::ONE);
void AddVertsForAABB2D(VertexList_PCU& verts, Vec2 const& aabbMins, Vec2 const& aabbMaxs, Rgba8 const& color, Vec2 const& uvMins = Vec2::ZERO, Vec2 const& uvMaxs = Vec2::ONE);
void AddVertsForOBB2D(VertexList_PCU& verts, Vec2 const& obb2Center, Vec2 const& obb2IBasisNormal, Vec2 const& obb2HalfDimensions, Rgba8 const& color);
void AddVertsForCapsule2D(VertexList_PCU& verts, Vec2 const& capsuleStartPosition, Vec2 const& capsuleEndPosition, float capsuleRadius, Rgba8 const& color);
void AddVertsForCapsule2D(VertexList_PCU& verts, Capsule2 const& capsule, Rgba8 const& color);
void AddVertsForHalfDisc2D(VertexList_PCU& verts, Vec2 const& discCenter, float discRadius, Rgba8 const& color, bool isTopHalf, float rotationDegrees);
void AddVertsForArrow2D(VertexList_PCU& verts, Vec2 const& tailPosition, Vec2 const& tipPosition, float arrowSize, float thickness, Rgba8 const& color);
...
void AddVertsForQuad3D(VertexList_PCU& verts, std::vector<unsigned int>& indexes, Vec3 const& bottomLeft, Vec3 const& bottomRight, Vec3 const& topLeft, Vec3 const& topRight, Rgba8 const& color = Rgba8::WHITE, AABB2 const& uv = AABB2::ZERO_TO_ONE);
...
void AddVertsForAABB3D(VertexList_PCUTBN& verts, std::vector<unsigned int>& indexes, AABB3 const& bounds, Rgba8 const& color = Rgba8::WHITE, AABB2 const& UVs = AABB2::ZERO_TO_ONE);
