...
struct sSmartRotationConfig
{
    ...
    static sSmartRotationConfig FromJSON(nlohmann::json const& j)
    {
        ...
        // File rotation thresholds
        if (j.contains("maxFileSizeMB")) config.maxFileSizeBytes = j["maxFileSizeMB"].get<size_t>() * 1024 * 1024;
        if (j.contains("maxTimeIntervalHours")) config.maxTimeInterval = std::chrono::hours(j["maxTimeIntervalHours"].get<int>());

        // File management
        if (j.contains("logDirectory")) config.logDirectory = j["logDirectory"].get<std::string>();
        if (j.contains("currentLogName")) config.currentLogName = j["currentLogName"].get<std::string>();
        if (j.contains("sessionPrefix")) config.sessionPrefix = j["sessionPrefix"].get<std::string>();
        if (j.contains("organizeDateFolders")) config.organizeDateFolders = j["organizeDateFolders"].get<bool>();

        // Cleanup and retention
        if (j.contains("retentionDays")) config.retentionHours = std::chrono::hours(j["retentionDays"].get<int>() * 24);
        if (j.contains("maxArchivedFiles")) config.maxArchivedFiles = j["maxArchivedFiles"].get<size_t>();
        if (j.contains("maxTotalArchiveSizeMB")) config.maxTotalArchiveSizeMB = j["maxTotalArchiveSizeMB"].get<size_t>();

        // Emergency disk space thresholds
        if (j.contains("diskSpaceWarningGB")) config.diskSpaceWarningGB = j["diskSpaceWarningGB"].get<double>();
        if (j.contains("diskSpaceCriticalGB")) config.diskSpaceCriticalGB = j["diskSpaceCriticalGB"].get<double>();
        if (j.contains("diskSpaceEmergencyGB")) config.diskSpaceEmergencyGB = j["diskSpaceEmergencyGB"].get<double>();
