...
// SmoothNoise.cpp
//
#include "ThirdParty/Noise/RawNoise.hpp"			// for raw bit-noise base functions (SquirrelNoise4)
...
{
	const float OCTAVE_OFFSET = 0.636764989593174f; // Translation/bias to add to each octave
	static const Vec2 gradients[8] = // Normalized unit vectors in 8 quarter-cardinal directions
	{
	...
	for (unsigned int octaveNum = 0; octaveNum < numOctaves; ++octaveNum)
	{
		...
		float dotSouthWest = DotProduct2D(gradientSW, displacementFromSW);
		float dotSouthEast = DotProduct2D(gradientSE, displacementFromSE);
		float dotNorthWest = DotProduct2D(gradientNW, displacementFromNW);
		float dotNorthEast = DotProduct2D(gradientNE, displacementFromNE);
...
float Compute3dPerlinNoise(float posX, float posY, float posZ, float scale, unsigned int numOctaves, float octavePersistence, float octaveScale, bool renormalize, unsigned int seed)
{
	...
	static const Vec3 gradients[8] = // Traditional "12 edges" requires modulus and isn't any better.
	{
	...
	for (unsigned int octaveNum = 0; octaveNum < numOctaves; ++octaveNum)
	{
		...
		float dotBelowSW = DotProduct3D(gradientBelowSW, displacementFromBelowSW);
		float dotBelowSE = DotProduct3D(gradientBelowSE, displacementFromBelowSE);
		float dotBelowNW = DotProduct3D(gradientBelowNW, displacementFromBelowNW);
		float dotBelowNE = DotProduct3D(gradientBelowNE, displacementFromBelowNE);
		float dotAboveSW = DotProduct3D(gradientAboveSW, displacementFromAboveSW);
		float dotAboveSE = DotProduct3D(gradientAboveSE, displacementFromAboveSE);
		float dotAboveNW = DotProduct3D(gradientAboveNW, displacementFromAboveNW);
		float dotAboveNE = DotProduct3D(gradientAboveNE, displacementFromAboveNE);
...
float Compute4dPerlinNoise(float posX, float posY, float posZ, float posT, float scale, unsigned int numOctaves, float octavePersistence, float octaveScale, bool renormalize, unsigned int seed)
{
	...
	static const Vec4 gradients[16] = // Hard to tell if this is any better in 4D than just having 8
	{
	...
	for (unsigned int octaveNum = 0; octaveNum < numOctaves; ++octaveNum)
	{
		...
		float dotBeforeBSW = DotProduct4D(gradientBeforeBSW, displacementFromBeforeBSW);
		float dotBeforeBSE = DotProduct4D(gradientBeforeBSE, displacementFromBeforeBSE);
		float dotBeforeBNW = DotProduct4D(gradientBeforeBNW, displacementFromBeforeBNW);
		float dotBeforeBNE = DotProduct4D(gradientBeforeBNE, displacementFromBeforeBNE);
		float dotBeforeASW = DotProduct4D(gradientBeforeASW, displacementFromBeforeASW);
		float dotBeforeASE = DotProduct4D(gradientBeforeASE, displacementFromBeforeASE);
		float dotBeforeANW = DotProduct4D(gradientBeforeANW, displacementFromBeforeANW);
		float dotBeforeANE = DotProduct4D(gradientBeforeANE, displacementFromBeforeANE);
		float dotAfterBSW = DotProduct4D(gradientAfterBSW, displacementFromAfterBSW);
		float dotAfterBSE = DotProduct4D(gradientAfterBSE, displacementFromAfterBSE);
		float dotAfterBNW = DotProduct4D(gradientAfterBNW, displacementFromAfterBNW);
		float dotAfterBNE = DotProduct4D(gradientAfterBNE, displacementFromAfterBNE);
		float dotAfterASW = DotProduct4D(gradientAfterASW, displacementFromAfterASW);
		float dotAfterASE = DotProduct4D(gradientAfterASE, displacementFromAfterASE);
		float dotAfterANW = DotProduct4D(gradientAfterANW, displacementFromAfterANW);
		float dotAfterANE = DotProduct4D(gradientAfterANE, displacementFromAfterANE);
